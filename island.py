...
generator_indices = [i for i, x in enumerate(buses) if x[1] == 1]
...
# 1st part: island_count (or) island id.
# 2nd part: list of island contituents. 1st element of it being the generator.
...
for gi in sorted_generators:
    ...
    # 
    # The list generated by below list comprehension will change
    # so we are not `storing it in a variable and reusing it`.
